...
var grCmJobUser = {
 	...
 	proxy: {
 		url: '/com/z09/selectCmJobUserListForAll.do',
 		param: {},
 		type: 'post',
 		dataType: 'json',
 		paging: false,
...
var grCmSancline = {
 	...
 	proxy: {
 		url: '/com/z09/selectCmSanClineDetailListByPk.do',
 		param: {},
 		type: 'post',
 		dataType: 'json',
 		paging: false,
...
$(function() {
	...
	$("#btnInit").click(function() {
		$.ajax({
			url : '/com/z09/selectCmSanClineListForComboBox.do',
			success : function(data) {
	...
	$("#btnCmSanclineRequest").click(function() {
		...
		if(!fn_hasValueByArr(arrSanctnCode, '01') ||
				!fn_hasValueByArr(arrSanctnCode, '07')) {
			alert('기안자와 결제자는 필수로 입력해 주십시오.');
			return;
		}
		
		if(fn_hasValueByArr(arrSanctnCode, '01') > 1 || fn_hasValueByArr(arrSanctnCode, '02') > 1 || 
				fn_hasValueByArr(arrSanctnCode, '07') > 1) {
			alert('기안, 결제, 조정은 1명만 선택해 주십시오.');
			return;
		}
		
		if(arrSanctnCode[0] != '01') {
			alert('기안은 맨처음에 있어야 합니다.');
			return;
		}
		
		if(!fn_checkGroupingByArr(arrSanctnCode)) {
			alert('합의, 순차합의, 참조는 그룹핑 시켜서 위치조정 해야 합니다.');
			return;
		}
		...
		$.ajax({
			url : '/com/z09/insertCmSanctnRequstByCmSanClineDetailInfo.do',
			type: 'post',
	...
	$("#btnCmSanclineManageMent").click(function() {
		mkLayerPopupOpen('/com/z09/popupCOMZ0902.do', null, 2);
	});
...
<body>
<div id="wrap">
<%-- <c:import url="/com/topHeader.do?MODULE_CODE=COM&MENU_ID=COMZ0901&UPPER_MENU_ID=COMZ" charEncoding="utf-8"/> --%>
<%-- <c:import url="/com/lnb.do?MODULE_CODE=COM&MENU_ID=COMZ0901&UPPER_MENU_ID=COMZ" charEncoding="utf-8"/> --%>
	<!-- 메인 시작 -->
...
.big_arrow_btn_02 { position:absolute; transition-duration:0s !important; left:48.6%; top:49%; padding:14px 5px 15px 5px !important; height:60px !important; background:#4c55a4 !important;  font-size: 20px !important; }
			...
			<div style="clear:both; text-align: center; padding:10px 10px 10px 0;">
				...
				<input type="button" id="btnCmSanclineClose" value="닫기">
			</div>
