...
var EDCE0101_grid = {
  //그리드 id로 AUI그리드 전용 메소드 사용 시 이용 ex) AUIGrid.selectedItem(EDCE0101_grid.id);
  id: '',
  //AUI그리드 생성할 div 및 페이징 옵션
  div: {
    gridDiv: '#EDCE0101_grid',
  },
  //데이터 연계 옵션
  proxy: {
	url : './selectESignVisitList.do',
    param: {},
    type: 'post',
    dataType: 'json',
    //페이징 true, false 로 지정
...
$(document).ready(function() {
	EDCE0101_grid.createGrid();
});


window.onresize = function() {
 if (typeof EDCE0101_grid.id !== "undefined") {
	 AUIGrid.resize(EDCE0101_grid.id);
   }
};
...
$(function(){
	...
	$('.popup_01').click(function(){
		var checkedItems = AUIGrid.getCheckedRowItems(EDCE0101_grid.id);
		if (checkedItems.length <= 0) {
			alert("일괄 서명할 데이터를 선택하세요.");
			// SAVE CODE 가 완료고 OPEN QUERY가 0인지 체크
		} else {
			var rows;
			for (var i = 0; i < checkedItems.length; i++) {
				...
				if(checkedItems[i].item.OPEN_QUERY != '0'){
					...
					if(i == 0){
						rows = checkedItems[i].item.VMODE_SN;
					}else{
						rows = rows + "," + checkedItems[i].item.VMODE_SN;
					}
				}
...
function searchClick() {
	...
	EDCE0101_grid.proxy.param = param;
	EDCE0101_grid.load();
}
</script>
...
<body>
<div id="wrap">
<c:import url="/com/topHeader.do?MODULE_CODE=EDC&MENU_ID=EDCE0101&&UPPER_MENU_ID=EDCE" charEncoding="utf-8"/>
<c:import url="/com/lnb.do?MODULE_CODE=EDC&MENU_ID=EDCE0101&&UPPER_MENU_ID=EDCE" charEncoding="utf-8"/>
            ...
            <section class="conBox100 conBox">
				...
				<form name="searchForm" id="searchForm">
					<div id="tab" class="tab tab01 mgT16">
						<ul>
							<li id="tab_01" class="port_back tab_menu">Visit</li>
							<li id="tab_02" class="tab_menu">Subject</li>
						</ul>
					</div>
					<article style="overflow:hidden;">
 					...
 					<article class="mgT16">
                        <div id="EDCE0101_grid" class="gridHeiSz15 grid_wrap tb01_2"></div>
                    </article>
                </form>
				</div>
            </section>
