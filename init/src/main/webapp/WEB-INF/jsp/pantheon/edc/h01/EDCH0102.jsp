...
var EDCH0102_grid = {
  ...
  columnLayout : [{
	...
	children:[{
	    ...
	    dataField : "SubjectNo",
	    headerText : "SubjectNo",
	    filter : {
	    	showIcon : true
	    },
	    width : 120
	  },{
	    dataField : "SamplingDate", 
	    headerText : "SamplingDate",
	    filter : {
	    	showIcon : true
	    },
	    width : 200
...
window.onresize = function() {
 if (typeof EDCH0102_grid.id !== "undefined") {
...
$(function(){
	...
	$("#btnSave").click(function() {
		if(EDCH0102_grid.failCount) {
			alert('<spring:message code="comc02.msg.2"/>');
			return;
		}
		
		var arrData = AUIGrid.getGridData(EDCH0102_grid.id);
		...
		$.ajax({
			url : "<c:url value='/edc/h01/insertLabData.do'/>",
			type: 'post',
			...
			success : function(response) {
				...
				if(data.success) {
					if(data.count) {
						...
						EDCH0102_grid.failCount = data.count;
					} else {
						$('#h6Validation').text('VALIDATION : SUCCESS');
						EDCH0102_grid.failCount = data.count;
					}
					AUIGrid.setGridData(EDCH0102_grid.id, data.result);
				} else {
					alert(data.message);
				}
			},
	...
	<div class="popup wd960">
		...
		<div class="popup_conBoxInner">
			...
			<article class="mgT5 pop_style">
				<div id="EDCH0102_grid" class="gridHeiSz07 grid_wrap tb01_2"></div>
			</article>
