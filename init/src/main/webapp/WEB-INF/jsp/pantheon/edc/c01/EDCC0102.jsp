...
function fnLoad(subjectNo) {
	...
	$.ajax({
		...
		async : false,
		data : param,
		url : "./selectVisitViewList.do",
		success :function(data){
			console.log(data.headerList);
			
			var total = data.headerList.length;
			$.each(data.headerList, function(index, value){
				headerVal +="<th>" + value.VISIT_LBL + "</th>";
				if(index === total -1){
					headerVal +="</tr>";
				}
			});
			
			resultVal = headerVal + data.resultList;
			$('#resultList').empty();
...
<body>
<div id="wrap">
<c:import url="/com/topHeader.do?MODULE_CODE=EDC&MENU_ID=EDCC0102&&UPPER_MENU_ID=EDCC" charEncoding="utf-8"/>
<c:import url="/com/lnb.do?MODULE_CODE=EDC&MENU_ID=EDCC0102&&UPPER_MENU_ID=EDCC" charEncoding="utf-8"/>
							...
							<section class="contents_02">
								<div class="contents_02_in">
									<article class="detail_wrap_01">
										...
										<table class="tb002" style="width:1300px;">
											<tbody id="resultList">
												<tr id="headerList">
												</tr>
											</tbody>
