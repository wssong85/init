...
<script type="text/javascript">
	var EDCK0206_grid = {
		...
		div : {
			gridDiv : "EDCK0206_grid",
			paging : {
				pagingDiv : 'EDCK0206_grid_paging',
				totalRowCount : 500,
				rowCount : 10,
				pageButtonCount : 4,
				currentPage : 1,
		...
		columnLayout : [ {
			...
			headerText : '<spring:message code="edc.lbl.CDISC_SE_CODE" />',
			editable : false
		}, {
	...
	var EDCK0206_grid2 = {
		...
		//AUI그리드 생성할 div 및 페이징 옵션
		div : {
			gridDiv : "EDCK0206_grid2",
			paging : {
				pagingDiv : 'EDCK0206_grid2_paging',
				totalRowCount : 500,
				rowCount : 10,
				pageButtonCount : 4,
				currentPage : 1,
	...
	function popup_newDetail() {
		AUIGrid.clearSelection(EDCK0206_grid.id, EDCK0206_grid.gridPros.selectionMode);
		...
		$.ajax({
			...
			success : function(data, textStatus, jqXHR) {
				if (data.success) {
					...
					EDCK0206_grid2.proxy.param = popgrid_param;
					EDCK0206_grid2.load();
				} else {
					alert(data.message);
				}
	...
	function popup_save() {
		AUIGrid.forceEditingComplete(EDCK0206_grid2.id, null, false);
		var ret = null;
		...
		if (ret != null) {
			...
			if (confirm(msg)) {
				...
				var rowItems = AUIGrid.getGridData(EDCK0206_grid2.id);
				for (var i = 0; i < rowItems.length; i++) {
					var item = {
						VRIABL_OPTN_ORDR : (i + 1)
					};
					AUIGrid.updateRow(EDCK0206_grid2.id, item, i, true);
				}

				var arJson = new Array();
				var rowItems = AUIGrid.getAddedRowItems(EDCK0206_grid2.id);
				fn_makeArr(arJson, rowItems, "C");
				rowItems = AUIGrid.getEditedRowItems(EDCK0206_grid2.id);
				fn_makeArr(arJson, rowItems, "U");
				rowItems = AUIGrid.getRemovedItems(EDCK0206_grid2.id);
				...
				$.ajax({
					...
					success : function(data, textStatus, jqXHR) {
						if (data.success) {
							EDCK0206_grid.load();
							popup_newDetail();
							AUIGrid.setGridData(EDCK0206_grid2.id, []);
						} else {
							alert(data.message);
						}
					},
	...
	$(function() {
		...
		EDCK0206_grid.proxy.param = popgrid_param;
		EDCK0206_grid.createGrid();
		popup_getDomainSelectbox();

		EDCK0206_grid2.createGrid();
		AUIGrid.setGridData(EDCK0206_grid2.id, []);
	...
	function popup_addRow() {
		...
		AUIGrid.addRow(EDCK0206_grid2.id, item, "first");
	}
	// 선택 행들 위로 한 단계 올림
	function popup_moveRowsToUp() {
		AUIGrid.moveRowsToUp(EDCK0206_grid2.id);
	};

	// 선택 행들 아래로 한 단계 올림
	function popup_moveRowsToDown() {
		AUIGrid.moveRowsToDown(EDCK0206_grid2.id);
	};
</script>
		...
		<section class="conBox100 popup_conBox" style="margin: 0px;">
			<div class="conBoxInner">
				<div id="tab2" class="tab tab01">
					<ul>
						<li id="tab_menu1" class="port_back2 tab_menu2">User Variable</li>
						<li id="tab_menu2" class="tab_menu2" onclick="popupUserVariable('EDCK0205');">SUPP</li>
					</ul>
				</div>
				<article>
					<!-- 에이유아이 그리드가 이곳에 생성 -->
					<div id="EDCK0206_grid" class="gridHeiSz04 grid_wrap tb01_2"></div>
				</article>
				<article class="mgT10">
					...
					<article class="mgT10">
						<form id="frm_popup" name="frm_popup">
							...
							<input type="hidden" id="SUPP_DOMN" name="SUPP_DOMN" value="01">
					...
					<article class="mgT10">
						...
						<article class="conTitBtnR">
							...
							<button type="button" able onclick="fn_deleteRow(EDCK0206_grid2)">
					...
					<article class="mgT10">
						<article>
							<div id="EDCK0206_grid2" class="gridHeiSz04 grid_wrap tb01_2"></div>
						</article>
					</article>
