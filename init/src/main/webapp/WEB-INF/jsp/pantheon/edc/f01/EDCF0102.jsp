...
var grid_subjectList = {
	 	id: '',
	 	div: {
			gridDiv : 'grid_subjectList',
			paging : {
	 	...
	 	columnLayout : [{ 
							...
							headerText : "FREEZING_STTUS_CODE",
							editable : false,
							visible : false
						},{ 
							dataField : "INSTT_NM",
	 		    ...
	 		    success:function(data){
	 		    	if(data.success) {
						...
						AUIGrid.setGridData(me.id, data.result);
	 		    		...
	 		    		AUIGrid.removeAjaxLoader("#grid_subjectList");
	 		    		alert(data.message);
	 		    	}
	 		    },
...
$(function(){
	...
	$('#btnSearch').click(function(){
		...
		grid_subjectList.proxy.param = params;
		grid_subjectList.load();
	})
});
...
function fn_sdvPrcss(v){
	...
	var items = AUIGrid.getCheckedRowItems(grid_subjectList.id);
	if(items.length <= 0){
		alert("Site를 선택해 주세요.");
		return;
	}
	...
	// 선택된 행 아이템들(배열)		
	var editedItems = AUIGrid.getCheckedRowItems(grid_subjectList.id);
	...
	$.ajax({
		  ...
		  success : function(data, textStatus, jqXHR) {
			  if(data.success) {					
				  ...
				  AUIGrid.removeAjaxLoader(grid_subjectList.id);
				  alert(data.message);
			  }
		  }, 
		  error : function(jqXHR, textStatus, errorThrown){
...
window.onresize = function(){
	if(grid_subjectList.id !== "undefine"){
		AUIGrid.resize(grid_subjectList.id);
	}
}
...
<body>
<div id="wrap">
<c:import url="/com/topHeader.do?MODULE_CODE=EDC&MENU_ID=EDCF0102&&UPPER_MENU_ID=EDCF" charEncoding="utf-8"/>
<c:import url="/com/lnb.do?MODULE_CODE=EDC&MENU_ID=EDCF0102&&UPPER_MENU_ID=EDCF" charEncoding="utf-8"/>
	<section id="mainCon">
            ...
            <section class="conBox100 conBox">
				<div class="conBoxInner">
					...
					<article style="overflow:hidden;">
 					...
 					<article class="mgT16">
                        <div id="grid_subjectList" class="gridHeiSz15 grid_wrap tb01_2"></div>
