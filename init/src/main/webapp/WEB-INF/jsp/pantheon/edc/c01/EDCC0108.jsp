...
<script type="text/javascript">
	...
	$(document).ready(function() {
		EDCC0108_grid.createGrid();
		EDCC0108_grid2.createGrid();
		EDCC0108_grid3.createGrid();
		EDCC0108_grid4.createGrid();
	...
	function getCRFPage(visit_sn, sn, visit_group_sn, subject_count, visit_count, group_count) {
		...
		EDCC0108_grid.proxy.param = param;
		EDCC0108_grid.load();
		var layout = fn_makeCRFObject(EDCC0108_grid);
		makeLayoutSDV(layout, "crf_layout", 1, EDCC0108_grid);

		EDCC0108_grid2.proxy.param = param;
		EDCC0108_grid2.load();
		fn_showComment(EDCC0108_grid2, "crf_layout");

		EDCC0108_grid3.proxy.param = param;
		EDCC0108_grid3.load();
		showQuery(EDCC0108_grid3, "crf_layout", 1, EDCC0108_grid);

		EDCC0108_grid4.proxy.param = param;
		EDCC0108_grid4.load();
		showSDV(EDCC0108_grid4, "crf_layout");

		statusControl("crf_layout");
	...
	function statusControl(crf_id) {
		...
		$.ajax({
			...
			success : function(data, textStatus, jqXHR) {
				if (data.success) {
					var status = "";
					if (data.result && data.result.SDV_COMPT_STTUS_CODE == "03") {
						status = data.result.SDV_COMPT_STTUS_CODE;
						$("#" + crf_id + " #btn_completion").hide();
						$("#" + crf_id + " #btn_completion2").show();
					} else {
						$("#" + crf_id + " #btn_completion").show();
						$("#" + crf_id + " #btn_completion2").hide();
					}
					$("#" + crf_id + " #completion").text(status);
				} else {
					alert(data.message);
				}
			},
	...
	function makeLayoutSDV(sArr, crf_id, popup_order, grid) {
		grid = (grid.id).replace("#", "");

		var completion = "<div>";
		completion += "<table class='tb004'>";
		completion += "<colgroup><col width='120px' /><col width='' /></colgroup>";
		completion += "<tr><th>SDV Compeltion</th>";
		completion += "<td id='completion'></td>";
		completion += "</tr></table></div>";
		$("#" + crf_id).append(completion);
			...
			if (entry.SET_TY_CODE == "01") {
				...
				$(entry.row).each(function(index2, entry2) {
					$(entry2.varRow).each(function(index3, entry3) {
						...
						str += "<input type='checkbox' id='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' name='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' />";
						str += "<label for='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' class='chcLab'></label>";
						str += "</th>";
				...
				$(entry.row).each(function(index2, entry2) {
					$(entry2.varRow).each(function(index3, entry3) {
						...
						if (index2 == 0 && index3 == 0) {
							...
							str += "<th style='width : 30px;' >";
							str += "<input type='checkbox' id='set_sdv_" + entry.SET_SN + "_" + crf_id + "' name='set_sdv_" + entry.SET_SN + "_" + crf_id + "' onclick=\"checkTableSDV(" + entry.SET_SN + ", 'ALL' , '" + crf_id + "');\" />";
							str += "<label for='set_sdv_" + entry.SET_SN + "_" + crf_id + "' class='chcLab'></label>";
							str += "</th>";
						...
						str += "<th style='width : 30px;' >";
						str += "<input type='checkbox' id='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' name='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' onclick=\"checkTableSDV(" + entry.SET_SN + ", 'ONE' , '" + crf_id + "' );\" />";
						str += "<label for='sdv_" + entry2.SET_ROW_SN + "_" + entry3.VRIABL_ROW_SN + "_" + crf_id + "' class='chcLab'></label>";
						str += "</th>";
	...
	function checkTableSDV(set_sn, type, crf_id) {
		if (type == 'ALL') {
			if ($("#" + crf_id + " input[id=set_sdv_" + set_sn + "_" + crf_id + "]").is(":checked")) {
				$("#" + crf_id + " #div_set" + set_sn).find("input[id^=sdv_]").each(function(index, entry) {
			...
			if (b) {
				$("#" + crf_id + " input[id=set_sdv_" + set_sn + "_" + crf_id + "]").prop("checked", true);
			} else {
				$("#" + crf_id + " input[id=set_sdv_" + set_sn + "_" + crf_id + "]").prop("checked", false);
			}
		}
	}

	function showSDV(grid4, crf_id) {
		var rowItems = AUIGrid.getGridData(grid4.id);
		for (var i = 0; i < rowItems.length; i++) {
			if (rowItems[i].SDV_AT == 'Y') {
				var checkbox = $("#" + crf_id + " input[id=sdv_" + rowItems[i].SET_ROW_SN + "_" + rowItems[i].VRIABL_ROW_SN + "_" + crf_id + "]");
				checkbox.prop("checked", true);
			}
		}

		$("#" + crf_id + " div[name=div_set]").each(function(index, entry) {
			var c = true;
			var d = false;
			$(this).find("input[id^=sdv_]").each(function(index, entry) {
				if ($(this).is(":disabled")) {
					d = true;
				}
				if (!$(this).is(":checked")) {
			...
			if (d) {
				$(this).find("input[id^=set_sdv_]").each(function(index2, entry2) {
		...
		for (var i = 0; i < rowItems.length; i++) {
			if (rowItems[i].QUERY_STTUS_CODE == "01" || rowItems[i].QUERY_STTUS_CODE == "02") {
				if (rowItems[i].QUERY_CN && rowItems[i].QUERY_CN != "") {
					...
					var str = "";
					str = "<tr onclick='popupQueryOpen(\"" + rowItems[i].SET_ROW_SN + "\" , \"" + rowItems[i].VRIABL_ROW_SN + "\", \"END\" ," + rowItems[i].QUERY_SN + ", " + popup_order + ")'>";
					if (query_group == null || query_group != rowItems[i].QUERY_GROUP) {
						if (i != 0) {
							border = "border-top:1px solid #cccccc !important;";
						}
						str = "<tr style='" + border + "' onclick='popupQueryOpen(\"" + rowItems[i].SET_ROW_SN + "\" , \"" + rowItems[i].VRIABL_ROW_SN + "\", \"END\" ," + rowItems[i].QUERY_SN + ", " + popup_order + ", " + grid + ")'>";
	...
	function saveSDV(crf_id, grid4) {
		$("#" + crf_id + " input[id^=sdv_]").each(function(index, entry) {
			var value = "N";
			if ($(this).is(":checked")) {
				value = "Y";
			}
			...
			for (var i = 0; i < rowItems.length; i++) {
				if (rowItems[i].SET_ROW_SN == arr[1] && rowItems[i].VRIABL_ROW_SN == arr[2]) {
					if (rowItems[i].SDV_AT != value) {
						rowItems[i].SDV_AT = value
						AUIGrid.updateRowsById(grid4.id, rowItems[i]);
					}
					break;
				}
			}
		...
		var arJson = [];
		var rowItems = AUIGrid.getEditedRowItems(grid4.id);
		fn_makeArr(arJson, rowItems, "U");

		if (arJson.length > 0) {
			$.ajax({
				url : './insertSDVList.do',
				...
				success : function(data, textStatus, jqXHR) {
					if (data.success) {
						grid4.load();
						showSDV(grid4, crf_id);

						statusControl(crf_id);
						alert("저장되었습니다.");
					} else {
						alert(data.message);
					}
	...
	function saveCompletion(sdv_compt_sttus_code, crf_id, grid4) {
		var rowItems = AUIGrid.getRowsByValue(grid4.id, "SDV_AT", "N");
		if (rowItems.length > 0) {
			alert("모든 SDV 체크 필요");
		} else {
			//var param = $("#frm_main").serialize();
			var param = fn_serializeObject($("#frm_main"));
			param.SDV_COMPT_STTUS_CODE = sdv_compt_sttus_code;
			$.ajax({
				url : './insertSDVcompletion.do',
				data : param,
				type : 'post',
				//contentType : "application/json; charset=UTF-8",
				success : function(data, textStatus, jqXHR) {
	...
	function popupCRFPage() {
		var param = fn_serializeObject($("#frm_main"));
		mkLayerPopupOpen("./popupEDCC0111.do", param, 1);
	}
</script>
</head>
<body>
	<div id="wrap">
		<c:import url="/com/topHeader.do?MODULE_CODE=EDC&MENU_ID=EDCC0108&UPPER_MENU_ID=EDCC" charEncoding="utf-8" />
		<c:import url="/com/lnb.do?MODULE_CODE=EDC&MENU_ID=EDCC0108&UPPER_MENU_ID=EDCC" charEncoding="utf-8" />
						...
						<div class="contents_01_in">
							...
							<div class="right_002" style="position: relative; height: 36px;">
								...
								<div style="position: absolute; right: 0px; top: 0px;">
									<button type="button" id="btn_sdv" onclick="saveSDV('crf_layout', EDCC0108_grid4);">SDV</button>
									<button type="button" id="btn_completion" onclick="saveCompletion('03', 'crf_layout', EDCC0108_grid4);">SDV Completion</button>
									<button type="button" id="btn_completion2" onclick="saveCompletion('01', 'crf_layout', EDCC0108_grid4);">SDV Completion 해제</button>
								</div>
							</div>
							...
							<div id="EDCC0108_grid" class="gridHeiSz05 grid_wrap tb01_2 " style="display: none;"></div>
							<div id="EDCC0108_grid2" class="gridHeiSz05 grid_wrap tb01_2 " style="display: none;"></div>
							<div id="EDCC0108_grid3" class="gridHeiSz05 grid_wrap tb01_2 " style="display: none;"></div>
							<div id="EDCC0108_grid4" class="gridHeiSz05 grid_wrap tb01_2 " style="display: none;"></div>
						</div>
					</section>
				</section>
