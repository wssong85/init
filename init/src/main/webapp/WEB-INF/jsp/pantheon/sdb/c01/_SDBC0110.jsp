...
function btnInit(){
	...
	$('#btnExcel').click(function() {
		...
		var data = AUIGrid.getGridData(grICSR.id);
		
		if(data.length) {
			var exportProps = {
					fileName : "Study_List"
			};
			AUIGrid.setProperty(grICSR.id, "exportURL", '/com/z02/downloadAuiGridFile.do');
			AUIGrid.exportToXlsx(grICSR.id, true, exportProps);
		} else {
			alert('데이터 확인');
		}
	});
...
var grICSR = {
 	//그리드 id로 AUI그리드 전용 메소드 사용 시 이용 ex) AUIGrid.selectedItem(grICSR.id);
 	id: '',
 	...
 	binding: function() {
	  ...
	  createPagingNavigator: function(goPage) {
		...
		retStr += "<a href='javascript:grICSR.moveToPage(1)'><span class='aui-grid-paging-number aui-grid-paging-first'> << </span></a>";

		// 이전
		retStr += "<a href='javascript:grICSR.moveToPage(" + prevPage + ")'><span class='aui-grid-paging-number aui-grid-paging-prev'> < </span></a>";

		for (var i=(prevPage+1), len=(me.div.paging.pageButtonCount+prevPage); i<=len; i++) {
			if (goPage == i) {
				retStr += "<span class='aui-grid-paging-number aui-grid-paging-number-selected'>" + i + "</span>";
			} else {
				retStr += "<a href='javascript:grICSR.moveToPage(" + i + ")'><span class='aui-grid-paging-number'>";
				retStr += i;
				retStr += "</span></a>";
			}
		...
		// 다음
		retStr += "<a href='javascript:grICSR.moveToPage(" + nextPage + ")'><span class='aui-grid-paging-number aui-grid-paging-next'> > </span></a>";

		// 마지막
		retStr += "<a href='javascript:grICSR.moveToPage(" + me.div.paging.totalPage + ")'><span class='aui-grid-paging-number aui-grid-paging-last'> >> </span></a>";
		retStr += "</span>";
...
function auiGridCellDoubleClickHandler() {
	...
	selectionMode = AUIGrid.getProperty(grICSR.id, "selectionMode");
	var selectedItems = AUIGrid.getSelectedItems(grICSR.id);
...
window.onresize = function() {
	...
	if (typeof grICSR.id !== "undefined") {
	 	AUIGrid.resize(grICSR.id);
	}
};
</script>
</head>
				...
				<div class="conBoxInner">
					...
					<div class="button_tab_01">
						<ul>
							...
							<li id="tabICSR" class="port_back tab_menu">ICSR</li>
						</ul>
					</div>
					...
					<article class="conGridUpper">
						<div class="conTitBtnL">
							...
							<input type="text" id="txtProductCode" popup_type="ProductSearch" valireqire="PRODUCT" class="pdR15" readonly>
					...
					<article class="mgT43">
	                       <div id="grICSR" class="gridHeiSz10 grid_wrap tb01_2"></div>
