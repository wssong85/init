	...
	<script type="text/javascript">
		var gridSop = {
		 	...
		 	columnLayout : [{
								...
								dataField : "EFECT_OCCRRNC_AT_VALUE_RADIO",
								headerText : "효력발생여부",
						 		filter : {
						 			showIcon : true
						 		}
		...
		function initPageSetting(){
			...
			$('#ajaxForm').ajaxForm({
				...
				success: function(response,status){
					...
					if(data.result) {
						var fileSn = $("#hFileFn").val() == "" ? data.filesn : $("#hFileFn").val();
						var fileFlag = $("#hFileFn").val() == "" ? "false" : "true";
						var param = {
							SJ : $("#txtTitle").val(),
							CN : $("#txtContent").val(),
							EFECT_OCCRRNC_AT : $(":radio[name=flag]:checked").val(),
							EFECT_OCCRRNCDE : $("#txtEffectDate").val().replace(/-/gi, ""),
							SE_CODE : "01",
							FILE_MASTR_SN : fileSn,
							FILE_FLAG : fileFlag,
							SN : $("#hSN").val(),
							DELFILE : $("#hDeleteFile").val()
						};
...
// 						console.log(multi_selector.count);
					...
					<table class="tb001">
						...
						<tbody>
							<tr>
								...
								<td><input type="text" id="txtTitle" valireqire="제목" style="width:100%;"/></td>
							</tr>
							<tr>
								<th>내용</th>
								<td style="padding-top:5px; padding-bottom: 0px;">
									<textarea id="txtContent" rows="3" style="width:100%;"></textarea>
								</td>
							</tr>
							<tr>
								<th>효력발생여부</th>
							...
							<tr>
								...
								<td>
									<div class="dateBox">
										<input type="text" id="txtEffectDate" style="width:114px;"/>
									</div>
								</td>
							</tr>
							<tr>
								...
								<td style=" padding-top: 5px;">
									<form id=ajaxForm action="<c:url value='/sdb/f04/insertSopFile.do'/>" method="post" enctype="multipart/form-data">
										<input type="file" name="file_1"  id="egovComFileUploader" size="60" title="첨부파일" />
										<p>
											<div id="egovComFileList"></div>
										</p>
										<input type="hidden" id="hFileFn" name="hFileFn" value="33" />
									</form>
