...
$(function() {
	...
	$('#btnAdd').click(function() {
		window.location.href = '/sdb/c01/_SDBC0109.do';
	});
});
...
function btnInit() {
	...
	//$('button[popup_type=ProductSearch]').trigger('click');	//임시
	...
	$('#btnExcel').click(function() {
		...
		var data = AUIGrid.getGridData(grTrial.id);
		
		if(data.length) {
			var exportProps = {
					fileName : "Study_List"
			};
			AUIGrid.setProperty(grTrial.id, "exportURL", '/com/z02/downloadAuiGridFile.do');
			AUIGrid.exportToXlsx(grTrial.id, true, exportProps);
		} else {
			alert('데이터 확인');
		}
	});
...
var grTrial = {
  //그리드 id로 AUI그리드 전용 메소드 사용 시 이용 ex) AUIGrid.selectedItem(grTrial.id);
  id: '',
  ...
  createPagingNavigator: function(goPage) {
	...
	retStr += "<a href='javascript:grTrial.moveToPage(1)'><span class='aui-grid-paging-number aui-grid-paging-first'> << </span></a>";

	// 이전
	retStr += "<a href='javascript:grTrial.moveToPage(" + prevPage + ")'><span class='aui-grid-paging-number aui-grid-paging-prev'> < </span></a>";

	for (var i=(prevPage+1), len=(me.div.paging.pageButtonCount+prevPage); i<=len; i++) {
		if (goPage == i) {
			retStr += "<span class='aui-grid-paging-number aui-grid-paging-number-selected'>" + i + "</span>";
		} else {
			retStr += "<a href='javascript:grTrial.moveToPage(" + i + ")'><span class='aui-grid-paging-number'>";
			retStr += i;
			retStr += "</span></a>";
		}
	...
	// 다음
	retStr += "<a href='javascript:grTrial.moveToPage(" + nextPage + ")'><span class='aui-grid-paging-number aui-grid-paging-next'> > </span></a>";

	// 마지막
	retStr += "<a href='javascript:grTrial.moveToPage(" + me.div.paging.totalPage + ")'><span class='aui-grid-paging-number aui-grid-paging-last'> >> </span></a>";
	retStr += "</span>";
...
function auiGridCellDoubleClickHandler() {
	...
	selectionMode = AUIGrid.getProperty(grTrial.id, "selectionMode");

	var selectedItems = AUIGrid.getSelectedItems(grTrial.id);
...
window.onresize = function() {
 ...
 if (typeof grTrial.id !== "undefined") {
   AUIGrid.resize(grTrial.id);
   }
};
		...
		<div class="mainConInner">
            ...
            <section class="conBox100 conBox">
				<div class="conBoxInner">
					<form id="iform" method="post" action="/sdb/c01/SDBC0109.do">
					<input type="hidden" name="ETC_INFO_NO" id="iETC_INFO_NO" />
						...
						<article class="mgT43">
	                        <div id="grTrial" class="gridHeiSz10 grid_wrap tb01_2"></div>
