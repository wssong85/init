...
<title>Spontaneous 조회/리스트</title>
<script type="text/javascript">

$(function(){
	btnInit();
	//grSpontaneous_list.createGrid();
	
	$('#btnSearch').click(function(){
		grSpontaneous_list.load();
	});
...
var grSpontaneous_list = {
 	//그리드 id로 AUI그리드 전용 메소드 사용 시 이용 ex) AUIGrid.selectedItem(grSpontaneous_list.id);
 	id: '',
 	//AUI그리드 생성할 div 및 페이징 옵션
 	div: {
 		gridDiv: '#grSpontaneous_list',
 		paging: {
 	...
 	columnLayout : [{
 		dataField : "SOURC_NO",
 		headerText : "Source No.",
		filter : {
 			showIcon : true
 		}
 	},{
 		dataField : "INITIAL_EN",
 		headerText : "Initial",
		filter : {
 			showIcon : true
 		}
 	...
 	binding: function() {
	  ...
	  createPagingNavigator: function(goPage) {
		...
		retStr += "<a href='javascript:grSpontaneous_list.moveToPage(1)'><span class='aui-grid-paging-number aui-grid-paging-first'> << </span></a>";

		// 이전
		retStr += "<a href='javascript:grSpontaneous_list.moveToPage(" + prevPage + ")'><span class='aui-grid-paging-number aui-grid-paging-prev'> < </span></a>";

		for (var i=(prevPage+1), len=(me.div.paging.pageButtonCount+prevPage); i<=len; i++) {
			if (goPage == i) {
				retStr += "<span class='aui-grid-paging-number aui-grid-paging-number-selected'>" + i + "</span>";
			} else {
				retStr += "<a href='javascript:grSpontaneous_list.moveToPage(" + i + ")'><span class='aui-grid-paging-number'>";
				retStr += i;
				retStr += "</span></a>";
			}
		...
		// 다음
		retStr += "<a href='javascript:grSpontaneous_list.moveToPage(" + nextPage + ")'><span class='aui-grid-paging-number aui-grid-paging-next'> > </span></a>";

		// 마지막
		retStr += "<a href='javascript:grSpontaneous_list.moveToPage(" + me.div.paging.totalPage + ")'><span class='aui-grid-paging-number aui-grid-paging-last'> >> </span></a>";
		retStr += "</span>";
...
function auiGridCellDoubleClickHandler() {
	...
	selectionMode = AUIGrid.getProperty(grSpontaneous_list.id, "selectionMode");
	var selectedItems = AUIGrid.getSelectedItems(grSpontaneous_list.id);
...
window.onresize = function() {
	...
	if (typeof grSpontaneous_list.id !== "undefined") {
	 	AUIGrid.resize(grSpontaneous_list.id);
	}
};
</script>
</head>
				...
				<div class="conBoxInner">
					...
					<div class="button_tab_01">
						<ul>
							...
							<li id="tabSpontaneous" class="port_back tab_menu">Spontaneous</li>
					...
					<article class="mgT43">
	                       <div id="grSpontaneous_list" class="gridHeiSz10 grid_wrap tb01_2"></div>
