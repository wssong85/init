...
<style>
.aui-grid-default-column{ text-indent: 0px;}
.aui-grid-default-column .aui-grid-renderer-base{ padding:0 0px 0px 10px !important; box-sizing: border-box;}
</style>
	...
	<script type="text/javascript">
		var gridEducation = {
		 	...
		 	//데이터 연계 옵션
		 	proxy: {
		 		url: '/sdb/f05/selectEducationInfo.do',
		 		param: {},
		 		type: 'post',
		 		dataType: 'json',
		 		//페이징 true, false 로 지정
...
//		  			    	console.log(data.count);
		...
		function buttonActive(){
			...
			$("#btnDown").click(function(){
				var data = AUIGrid.getGridData(gridEducation.id);
				
				if(data.length) {
					var exportProps = {
							fileName : "Education_List"
					};
					AUIGrid.setProperty(gridEducation.id, "exportURL", '/com/z02/downloadAuiGridFile.do');
					AUIGrid.exportToXlsx(gridEducation.id, true, exportProps);
				} else {
					alert('데이터 확인');
				}
			});
		...
		function deleteEducationInfo(){
			...
			$.ajax({
				url : '/sdb/f05/deleteEducationInfo.do',
				data : param,
				type : 'POST',
				dataType	: "json",
				success : function(data) {					
		...
		function auiGridCellClickHandler(){
			...
			var selectionMode = AUIGrid.getProperty(gridEducation.id, "selectionMode");
			
			var selectedItems = AUIGrid.getSelectedItems(gridEducation.id);	
...
// 			});
		...
		$(window).resize(function(){
			if(gridEducation.id){
				AUIGrid.resize(gridEducation.id);
			}
		});
	</script>
</head>
<body>
<div id="wrap">
	<c:import url="/com/topHeader.do?MODULE_CODE=SDB&MENU_ID=SDBF0501&UPPER_MENU_ID=SDBF" charEncoding="utf-8"/>
	<c:import url="/com/lnb.do?MODULE_CODE=SDB&MENU_ID=SDBF0501&UPPER_MENU_ID=SDBF" charEncoding="utf-8"/>	
	<section id="mainCon">
	<div class="mainConInner">
						...
						<tbody>
							...
							<tr>
								...
								<td style="padding-top:5px; padding-bottom: 0px;">
									<textarea id="txtContent" rows="3" maxLength="1333" style="width:100%;"></textarea>
								</td>
							</tr>
							<tr>
								<th>파일업로드</th>
		...
		<section class="conBox100 conBox">
			<div class="conBoxInner">
				...
				<article class="mgT16">
					<div id="grEducation" class="gridHeiSz07 grid_wrap tb01_2"></div>
				</article>
			</div>
		</section>
	</div>
