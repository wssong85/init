...
var grReportListGrid = {
	 	id: '',
	 	div: {
	 		gridDiv: '#grReportListGrid'
	 	},
	 	proxy: {
	 		url: '/ctms/z03/selectSiteVisitList.do',
	 		param: {'TASK_SN' : g_taskSn, 'REPORT_SE' : 'Z14'},
	 		type: 'post',
	 		dataType: 'json',
	 		//페이징 true, false 로 지정
	 		paging: false,
	 	...
	 	binding: function() {
	 		...
	 		AUIGrid.bind(me.id, "cellDoubleClick", function(event){
	 			...
	 			var objs ;
	 			var value ;
	 			var params = {  'MILESTONE_SN' : item.MILESTONE_SN
	 						  , 'TASK_SN'      : item.TASK_SN
	 						  , 'REPORT_TYPE'  : reportType
	 						  , 'REPORT_SE'    : item.REPORT_SE
	 						 };
...
$(document).ready(function() {
	grReportListGrid.createGrid();
	AUIGrid.resize(grReportListGrid.id);
}); 


function exportAs(){
// 	var chkbox = document.getElementById("chkbox");
// 	AUIGrid.exportToXlsx(grReportListGrid.id, chkbox.checked);
//	AUIGrid.setProperty(grReportListGrid.id, "exportURL", "./CTMZ0203.do");
	
}

window.onresize = function() {
	if (typeof grReportListGrid.id != "undefined") {
		AUIGrid.resize(grReportListGrid.id);
	}
}
</script>
...
<body>
<c:import url="/com/topHeader.do?MODULE_CODE=CTMS&MENU_ID=CTMZ0304&UPPER_MENU_ID=CTMZ" charEncoding="utf-8"/>
<c:import url="/com/lnb.do?MODULE_CODE=CTMS&MENU_ID=CTMZ0304&UPPER_MENU_ID=CTMZ" charEncoding="utf-8"/>
<section id="mainCon">
           ...
           <article class="mainTitBox">
            ...
            <section class="conBox100 conBox">
            	...
            	<article class="mainTitBox">
                    ...
                    <article class="mgT16">
                        <!-- 에이유아이 그리드가 이곳에 생성 -->
                         <div id="grReportListGrid" class="gridHeiSz17 grid_wrap tb01_2"></div>
                    </article>
                    <!-- 그리드 끝 -->
