...
$(function() {
	...
	$("#btnSave").click(function() {
		...
		if(grPopupEdcCrAtc.failCount) {
			alert('<spring:message code="comc02.msg.2"/>');
			return;
		}
		
		var arrData = AUIGrid.getGridData(grPopupEdcCrAtc.id);
		...
		$.ajax({
			url : "<c:url value='/com/c01/insertEdcCrAtcByExcelList.do'/>",
			type: 'post',
			contentType: "application/json; charset=UTF-8",
			data : JSON.stringify(arrData),
			success : function(data, textStatus, jqXHR) {
				if(data.success) {
					alert('<spring:message code="comc02.msg.4"/>');
					${callbackFunction}();
		...
		success: function(response,status){
			...
			if(data.success) {
				if(data.count) {
					...
					grPopupEdcCrAtc.failCount = data.count;
				} else {
					$('#h6Validation').text('VALIDATION : SUCCESS');
					grPopupEdcCrAtc.failCount = data.count;
				}
				AUIGrid.setGridData(grPopupEdcCrAtc.id, data.result);
			} else {
				alert(data.message);
			}
		},
...
$(window).resize(function() {
	if(grPopupEdcCrAtc && grPopupEdcCrAtc.id) {
		AUIGrid.resize(grPopupEdcCrAtc.id);
	}
});
</script>
...
<section class="popupWrap">
	<div class="popup wd760">
		...
		<div class="popup_conBoxInner">
			<h5><spring:message code="comc02.title.2" /></h5>
			<article class="mgT5">
				<table class="tb001">
					<colgroup>
						<col width="20%" />
					...
					<tbody>
						...
						<tr>
							...
							<td>
							<form id="frUpload" action="<c:url value='/com/c02/selectAtcListByExcel.do'/>" method="post" enctype="multipart/form-data">
			...
			<article class="mgT5">
 				<div id="grPopupEdcCrAtc" class="gridHeiSz08 grid_wrap tb01_2"></div>
			</article>
		</div>
		</section>
