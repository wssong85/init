...
<script>
var grPopupEdcCrKims = {
 	id: '',
 	failCount: 0,
 	div: {
 		gridDiv: '#grPopupEdcCrKims'
 	},
 	proxy: {
 		param: {},
 		type: 'post',
...
$(function() {
	...
	$("#btnSave").click(function() {
		...
		if(grPopupEdcCrKims.failCount) {
			alert('<spring:message code="comc02.msg.2"/>');
			return;
		}
		
		var arrData = AUIGrid.getGridData(grPopupEdcCrKims.id);
		...
		$.ajax({
			url : "<c:url value='/com/c02/insertEdcCrKimsByExcelList.do'/>",
			type: 'post',
			contentType: "application/json; charset=UTF-8",
			data : JSON.stringify(arrData),
			success : function(data, textStatus, jqXHR) {
				if(data.success) {
					alert('<spring:message code="comc02.msg.6"/>');
					${callbackFunction}();
		...
		success: function(response,status){
			...
			if(data.success) {
				if(data.count) {
					...
					grPopupEdcCrKims.failCount = data.count;
				} else {
					$('#h6Validation').text('VALIDATION : SUCCESS');
					grPopupEdcCrKims.failCount = data.count;
				}
				AUIGrid.setGridData(grPopupEdcCrKims.id, data.result);
			} else {
				alert(data.message);
			}
		},
...
$(window).resize(function() {
	if(grPopupEdcCrKims && grPopupEdcCrKims.id) {
		AUIGrid.resize(grPopupEdcCrKims.id);
	}
});
</script>
...
<section class="popupWrap">
	<div class="popup wd760">
		...
		<div class="popup_conBoxInner">
			<h5><spring:message code="comc02.title.4" /></h5>
			<article class="mgT5">
				<table class="tb001">
					<colgroup>
						<col width="20%" />
					...
					<tbody>
						...
						<tr>
							...
							<td>
							<form id="frUpload" action="<c:url value='/com/c02/selectKimsListByExcel.do'/>" method="post" enctype="multipart/form-data">
			...
			<article class="mgT5">
 				<div id="grPopupEdcCrKims" class="gridHeiSz08 grid_wrap tb01_2"></div>
			</article>
		</div>
		</section>
