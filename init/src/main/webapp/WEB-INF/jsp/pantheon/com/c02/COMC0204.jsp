...
var grEdcCrMdic = {
 	...
 	binding: function() {
		...
		AUIGrid.bind(me.id, 'selectionChange', function(e) {
			grEdcCrKims.proxy.param['DICARY_VER'] = e.selectedItems[0].item['DICARY_VER'];
			grEdcCrKims.proxy.param['DICARY_SE_CODE'] = e.selectedItems[0].item['DICARY_SE_CODE'];
			grEdcCrKims.load();
		});
 	},
 	load: function() {
...
var grEdcCrKims = {
 	...
 	proxy: {
 		url: "<c:url value='/com/c02/selectEdcCrKimsByPk.do'/>",
 		param: {},
 		type: 'post',
 		dataType: 'json',
 		paging: true,
 	...
 	columnLayout : [{
 		...
 		dataField : "GOODS_ENG",
 		headerText : "GOODS_ENG",
 		filter : {
 			showIcon : true
 		}
 	},{
 		dataField : "SLE_CMPNY",
 		headerText : "SLE_CMPNY",
 		filter : {
 			showIcon : true
 		}
 	},{
 		dataField : "SLE_CMPNY_ENG",
 		headerText : "SLE_CMPNY_ENG",
 		filter : {
 			showIcon : true
 		}
 	},{
...
$(function(){
	...
	$('#btnDownload').click(function(){
		window.open("<c:url value='/data/excel/KIMS.xlsx'/>");
	});
	
	// 팝업레이어
	$('.popup_01').click(function(){
		mkLayerPopupOpen("<c:url value='/com/c02/COMC0208.do'/>", {
...
$(window).resize(function() {
	...
	if(grEdcCrKims && grEdcCrKims.id) {
		AUIGrid.resize(grEdcCrKims.id);
	}
});
</script>
...
<body>
<div id="wrap">
<c:import url="/com/topHeader.do?MODULE_CODE=COM&MENU_ID=COMC0204&UPPER_MENU_ID=COMC" charEncoding="utf-8"/>
				...
				<div class="conBoxInner">
					...
					<div id="tab" class="tab tab01 mgT16">
						<ul>
							...
							<li id="tab_04" class="port_back tab_menu" onClick="">KIMS</li>
						</ul>
					</div>
            ...
            <section class="conBox100 conBox">
				<div class="conBoxInner">
                    ...
                    <article class="conTitBox">
 					...
 					<article class="mgT16">
                        <div id="grEdcCrKims" class="gridHeiSz10 grid_wrap tb01_2"></div>
                        <div id="pgEdcCrKims" class="aui-grid-paging-panel my-grid-paging-panel"></div>
                    </article>
