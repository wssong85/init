...
<script>
var grPopupEdcCrMeddra = {
 	id: '',
 	failCount: 0,
 	div: {
 		gridDiv: '#grPopupEdcCrMeddra'
 	},
 	proxy: {
 		param: {},
 		type: 'post',
...
$(function() {
	...
	$("#btnSave").click(function() {
		...
		if(grPopupEdcCrMeddra.failCount) {
			alert('<spring:message code="comc02.msg.2"/>');
			return;
		}
		
		var arrData = AUIGrid.getGridData(grPopupEdcCrMeddra.id);
		...
		$.ajax({
			url : "<c:url value='/com/c02/insertEdcCrMeddraByExcelList.do'/>",
			type: 'post',
			contentType: "application/json; charset=UTF-8",
			data : JSON.stringify(arrData),
			success : function(data, textStatus, jqXHR) {
				if(data.success) {
					alert('<spring:message code="comc02.msg.3"/>');
					${callbackFunction}();
		...
		success: function(response,status){
			...
			if(data.success) {
				if(data.count) {
					...
					grPopupEdcCrMeddra.failCount = data.count;
				} else {
					console.log(data);
					$('#h6Validation').text('VALIDATION : SUCCESS');
					grPopupEdcCrMeddra.failCount = data.count;
				}
				AUIGrid.setGridData(grPopupEdcCrMeddra.id, data.result);
			} else {
				alert(data.message);
			}
		},
...
$(window).resize(function() {
	if(grPopupEdcCrMeddra && grPopupEdcCrMeddra.id) {
		AUIGrid.resize(grPopupEdcCrMeddra.id);
	}
});
</script>
...
<section class="popupWrap">
	<div class="popup wd760">
		...
		<div class="popup_conBoxInner">
			<h5><spring:message code="comc02.title.1" /></h5>
			<article class="mgT5">
				<table class="tb001">
					<colgroup>
						<col width="20%" />
					...
					<tbody>
						...
						<tr>
							...
							<td>
							<form id="frUpload" action="<c:url value='/com/c02/selectMeddraListByExcel.do'/>" method="post" enctype="multipart/form-data">
			...
			<article class="mgT5">
 				<div id="grPopupEdcCrMeddra" class="gridHeiSz08 grid_wrap tb01_2"></div>
			</article>
		</div>
		</section>
