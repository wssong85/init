...
<title><spring:message code="comi02.title.1"/></title>
...
// 		$('#spanPwCheck').text("입력하신 비밀번호와 비밀번호확인이 일치하지 않습니다.");
		$('#spanPwCheck').text('<spring:message code="comi02.message.1"/>');
        return false;
    }

    if(ObjUserPassWord && !ObjUserPassWord.substring(0,1).match(/([a-zA-Z])/)) {
//     	$('#spanPwCheck').text("처음 시작하는 문자는 영문 대/소문자로 해주십시오.");
    	$('#spanPwCheck').text('<spring:message code="comi02.message.2"/>');
    	return false;
    }
...
//     	$('#spanPwCheck').text("비밀번호는 문자(영문 대/소문자), 숫자, 특수문자 중 2종 이상 혼용해서 6~16자리로 입력해주세요.");
    	$('#spanPwCheck').text('<spring:message code="comi02.message.3"/>');
        return false;
    }
...
// 		$('#spanPwCheck').text("비밀번호는 문자(영문 대/소문자), 숫자, 특수문자 중 2종 이상 혼용해서 6~16자리로 입력해주세요.");
		$('#spanPwCheck').text('<spring:message code="comi02.message.4"/>');
        return false;
    }
...
// 			$('#spanPwCheck').text("ID와 동일한 3 자 이상의 패스워드를 사용할 수 없습니다.");
			$('#spanPwCheck').text('<spring:message code="comi02.message.5"/>');
			return false;
		}
...
// 		$('#spanPwCheck').text("동일문자를 3번 이상 사용할 수 없습니다.");
		$('#spanPwCheck').text('<spring:message code="comi02.message.6"/>');
        return false;
    }
  
    if(SamePass_1 > 1 || SamePass_2 > 1 ) {
		$('#spanPwCheck').text('<spring:message code="comi02.message.7"/>');
        return false;
    }
...
// 			alert('아이디를 입력해 주십시오.');
			alert('<spring:message code="comi02.message.8"/>');
			return;
		}
...
// 			alert('새 패스워드를 입력해 주십시오.');
			alert('<spring:message code="comi02.message.9"/>');
			return;
...
// 			alert('새 패스워드를 확인해 주십시오.');
			alert('<spring:message code="comi02.message.10"/>');
			return;
		}
		
		if($('#sign_3').css('display') == 'none') {
// 			alert('패스워드를 확인해 주십시오.');
			alert('<spring:message code="comi02.message.11"/>');
			return;
		}
		
		if($('#sign_5').css('display') == 'none') {
// 			alert('패스워드를 확인해 주십시오.');
			alert('<spring:message code="comi02.message.12"/>');
			return;
		}
		
 		$.ajax({
 		    url: "<c:url value='/com/anonymous/updateCmUserForChgPasswordByPasswordCode.do'/>",
 		    type: 'post',
 		    dataType: 'json',
 		    data: {
 		    	'USER_ID' : $.trim($('#txtUserId').val()),
 		    	'PASSWORD_CODE': '${PASSWORD_CODE}',
 		    	'PASSWORD': $('#psPassword02').val()
 		    },
 		    success:function(data){
 		    	if(data.success) {
 		    		alert('<spring:message code="comi02.message.13"/>');
		...
		<div class="mainConInner">
            <section class="conBox100 conBox">
                ...
                <div class="conBoxInner">
                	<div class="join_01">
						<h4><spring:message code="comi02.title.1"/></h4>
	                        ...
	                        <div style="margin-top: 10px;">
								<table class="tb001">
									...
									<tbody>
										<tr>
											<th><spring:message code="comi02.th.1"/></th>
											<td>
												<input type="text" id="txtUserId" style="width:30%;">
											</td>
										</tr>
<!-- 										<tr> -->
<!-- 											<th>기존 패스워드</th> -->
...
<!-- 										</tr> -->
										<tr>
											<th><spring:message code="comi02.th.2"/></th>
											<td>
										...
										<tr>
											<th><spring:message code="comi02.th.3"/></th>
											<td>
												<input type="password" id="psPassword02" style="width:30%;">
								...
								<div class="agree_btn_01">
									<input type="button" id="btnPasswordChange" class="big_button_01"  value=<spring:message code="comi02.btn.1"/>>
								</div>
	                        </div>
