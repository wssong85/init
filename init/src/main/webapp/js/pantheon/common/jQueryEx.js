...
function POSTSend(url, id, val) {
    $("<form id='form2'><input type='hidden' name='"+ id +"' id='" + id + "'></input></form>")
...
$.ajaxSetup({
    ...
    beforeSend: function (xhr) {
        //this.data = this.data.replace(/%2B/g, '%EF%BC%8B');
        
        xhr.setRequestHeader("X-Ajax-call", "true");
        
        try {
        	
        	if(this.doNotAjaxSetup) {
        		return;
        	}
        ...
        $.ajax({
			type		: "POST",
			//url			: _BASE_URL_ + "/login/sessionCheck.do",
			dataType	: "json",
			beforeSend	: function (xhr) {
		        //this.data = this.data.replace(/%2B/g, '%EF%BC%8B');
		        //indicator(true);
			...
			success		: function(result){
				if(result.flag != "true"){
					alert("세션이 만료되어 로그아웃 되었습니다. 다시 로그인해 주십시오.");
					flag = 'true';
					windowClose();
				}
			}
...
/////////////////////////////////////////////////////////////////////////////
function mkSetDatePickerYYMMDD(id, ids, idNumber, idsNumber, pIdDate, pIdsDate) {
    ...
    if(pIdDate) {
    	idDate = new Date(pIdDate);
    }
    
    var idsDate = new Date();
    
    if(pIdsDate) {
    	pIdsDate = new Date(pIdsDate);
    }
    
    if(idNumber) {
    	var vTmp = idDate.getDate() + (idNumber);
    	idDate.setDate(vTmp);
    }
    
    if(idsNumber) {
    	var vTmp = idsDate.getDate() + (idsNumber);
    	idsDate.setDate(vTmp);
    }
    ...
    $(id).datepicker({
        ...
        onClose: function (dateText, inst) {
            ...
            if ($(id).val() > $(ids).val()) {
                mkMessageFunc("조회일자는 시작날짜가 종료날짜보다 이전이어야 합니다.", 1);
                $(id).val($.datepicker.formatDate($.datepicker.ATOM, new Date(inst.lastVal.replace(/-/gi, '-'))));
            }
    ...
    }).val($.datepicker.formatDate($.datepicker.ATOM, idDate));
    ...
    $(ids).datepicker({
    	...
    	onClose: function (dateText, inst) {
    		...
    		if ($(id).val() > $(ids).val()) {
    			mkMessageFunc("조회일자는 시작날짜가 종료날짜보다 이전이어야 합니다.", 1);
    			$(ids).val($.datepicker.formatDate($.datepicker.ATOM, new Date(inst.lastVal.replace(/-/gi, '-'))));
    		}
    		else if (!isValidDate($(this).val())) {
    			mkMessageFunc("유효한 날짜가 아닙니다.", 1);
    			$(this).val(tt2);
    ...
    }).val($.datepicker.formatDate($.datepicker.ATOM, idsDate));
}
...
///////////////////////////////////////////////////////////////////////////
//Function Name	: isDateTimeFormat
//Parameter Info	: 
//Return Info		: 
//Description		: 날짜/시간 데이터 포맷 체크
///////////////////////////////////////////////////////////////////////////
function isDateTimeFormat(d) {
 var regExpDate = /^(19[0-9][0-9]|20\d{2})-(0[0-9]|1[0-2])-(0[0-9]|[1-2][0-9]|3[0-1]) (0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/;
 //var regExpTelS = /^0(?:[2])(\-|)(?:\d{3}|\d{4})(\-|)\d{4}$/;
 //return d.match(df);
 return regExpDate.test(d);
}
