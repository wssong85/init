...
@Service("COMZ06Service")
public class COMZ06ServiceImpl implements COMZ06Service{
	...
	public void insertCmDtaAtrail(String tableNm, String[] columnNm, String[] labels, String tablePk,
		...
		for(String mapkey : columnNm) {
			...
			//타임 컬럼 체크
			else if(Util.isTimeConvert(mapkey)) {
				sb.append("DECODE(FN_CHECK_DATE("+mapkey+"), 'FALSE', TO_CHAR(TO_DATE(REPLACE(REPLACE(REPLACE(TRIM("+mapkey+"), '-'), ':'), ' '), 'YYYYMMDDHH24MISS'), 'YYYYMMDDHH24MISS'), 'TRUE', TO_CHAR("+mapkey+", 'YYYYMMDDHH24MISS')) AS "+mapkey+" ," );
			}  else {
				sb.append(mapkey + " ,");
			}
		}
		...
		for(String iStr : columnNm) {
			...
			for(Object mapkey : resultMap.keySet()){
				...
				if(Util.isTimeConvert(EgovStringUtil.isNullToString(mapkey))) {
					
					String strTimeConvert = EgovStringUtil.isNullToString(resultMap.get(mapkey));
					strTimeConvert = strTimeConvert.replaceAll("-", "");
					strTimeConvert = strTimeConvert.replaceAll(":", "");
					strTimeConvert = strTimeConvert.replaceAll("/", "");
					
					for(int i = 0; i < 14 - strTimeConvert.length(); i++) {
						strTimeConvert += "0";
					}
					
					resultMap.put(EgovStringUtil.isNullToString(mapkey), (Object)strTimeConvert);
				}
		...
		for(String columnMapkey : columnNm.keySet()) {
			...
			for(Object mapkey : resultMap.keySet()){
				...
				if(Util.isTimeConvert(EgovStringUtil.isNullToString(mapkey))) {
					...
					int count = strTimeConvert.length();
				...
				if(Util.isTimeConvert(EgovStringUtil.isNullToString(columnMapkey))) {
					String strTimeConvert = EgovStringUtil.isNullToString(columnNm.get(columnMapkey));
					strTimeConvert = strTimeConvert.replaceAll("-", "");
					strTimeConvert = strTimeConvert.replaceAll(":", "");
					strTimeConvert = strTimeConvert.replaceAll("/", "");
					...
					columnNm.put(columnMapkey, strTimeConvert);
				}
