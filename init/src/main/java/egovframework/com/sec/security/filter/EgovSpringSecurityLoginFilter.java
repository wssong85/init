...
//					loginVO = loginService.actionLoginByEsntlId(loginVO);
					...
					if (loginVO != null && loginVO.getUserId() != null && !loginVO.getUserId().equals("")) {
						...
						springSecurity.doFilter(new RequestWrapperForSecurity(httpRequest, loginVO.getUserId(), loginVO.getPassword()), httpResponse, chain);
...
//					if (password == null || password.equals("") || password.length() < 8 || password.length() > 20) {
					...
					try {
						...
						if (loginVO != null && loginVO.getUserId() != null && !loginVO.getUserId().equals("")) {
							...
							session.setAttribute(loginVO.getUserId(), listener);
